...
// Procesar la inserción del horario
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $dia = mysqli_real_escape_string($conn, $_POST['dia']);
    $hora = mysqli_real_escape_string($conn, $_POST['hora']);

    // Insertamos el horario seleccionado en la base de datos
    $query = "INSERT INTO horarios (dia, hora, usuario_id) VALUES ('$dia', '$hora', '$usuario_id')";

    if (mysqli_query($conn, $query)) {
        echo "<script>alert('Horario registrado correctamente'); window.location.href = 'tabla_confirmada.php';</script>";
    } else {
        echo "<script>alert('Error al registrar horario: " . mysqli_error($conn) . "'); window.location.href = 'horarios.php';</script>";
    }
}
...
<body>
    <div class="container">
        ...
        <!-- Formulario de inserción de horarios (solo para Vendedores y Clientes) -->
